...
describe('NoteService', () => {
  ...
  describe('createNote', () => {
    it('should create a note successfully', async () => {
      ...
      const expectedNote = { ...noteData, id: expect.any(String), update: expect.any(String) };
      
      mockNoteRepository.create.mockResolvedValue(expectedNote);

      const result = await noteService.createNote(noteData);

      expect(mockNoteRepository.create).toHaveBeenCalledWith(expect.objectContaining({
        ...noteData,
  ...
  describe('getNoteById', () => {
    ...
    it('should throw error when note not found', async () => {
      ...
      await expect(noteService.getNoteById(noteId)).rejects.toThrow('Nota no encontrada');
    });
  });
  ...
  describe('updateNote', () => {
    it('should update note successfully', async () => {
      ...
      expect(mockNoteRepository.findById).toHaveBeenCalledWith(noteId);
      expect(mockNoteRepository.update).toHaveBeenCalledWith(noteId, expect.objectContaining({
        title: 'Updated Title',
        update: expect.any(String)
      }));
    });
    ...
    it('should throw error when note not found', async () => {
      ...
      await expect(noteService.updateNote(noteId, updateData)).rejects.toThrow('Nota no encontrada');
    });
  });
  ...
  describe('deleteNote', () => {
    ...
    it('should throw error when note not found', async () => {
      ...
      await expect(noteService.deleteNote(noteId)).rejects.toThrow('Nota no encontrada');
    });
  });
});